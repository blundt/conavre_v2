...
// Opcional: no mostrar avisos PHP al usuario final
...
$umbral = 0.30;                       // score mínimo aceptado
if (!$captcha->success) {
    responder("Verificación reCAPTCHA fallida.", 403);
}
if ($captcha->action !== 'cotizar') {
    responder("Acción reCAPTCHA no coincide.", 403);
}
if ($captcha->score < $umbral) {
    responder("Score bajo de reCAPTCHA ({$captcha->score}).", 403);
}
...
// ─────────────────── Procesar adjuntos (opcional) ───────────────────
$adjunto = '';

if (
    isset($_FILES['archivo']) &&
    is_array($_FILES['archivo']['name']) &&
    !empty($_FILES['archivo']['name'][0])        // solo si se subió al menos un archivo
) {
